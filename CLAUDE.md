...
**Teacher v2 retraining (run3, 2026-05-11): in progress.** Five retrain-from-scratch attempts (varying env_cfg, seed, env count, working-tree state, IsaacLab toggles) all hit a "touch-don't-lift" basin where the policy farms shaped lift reward via buckled-thumb contact without grasping. Active changes under investigation: contact_mask gated by good_grasp_mask (thumb + ≥1 finger required, closes the buckled-thumb exploit), thumb_mcp_pitch/pip init moved off joint min (0 → 0.0524 rad/3°) to avoid PD oscillation against the hard stop, finger velocity_limit_sim 8 → 1.0 rad/s (closes sim2real gap + prevents single-step buckling), finger_curl_reg ADR (-1.5, -3.0) with cap -6.0.
...
- **Run3 retraining structural failure (2026-05-11)** — six consecutive retrain-from-scratch attempts (run3a-3h) all stuck in a "touch-don't-lift" basin where the policy farms shaped `lift_reward` via buckled-thumb contact without ever achieving a proper grasp. Same code/env_cfg that worked in April fails today. Suspected system-level drift below the dextrah_lab repo level (IsaacLab internals, omni.physx, driver). Visual livestream of failed policy confirms thumb buckles inward on approach despite documented fixes (thumb_rot at -20° init, thumb_mcp_pitch open) being in place. **The fix that broke run2g-era training**: lift_reward gate was `contact_mask = (contact_count > 0)` which fires on ANY single sensor contact — including the outer surface of a buckled thumb scraping the object. Current run3i uses `contact_mask = good_grasp_mask` (thumb + ≥1 other finger) to close this exploit. See exp_03 Run 3 section for full investigation.
...
- **Thumb joint init should NOT be at joint min** — `thumb_mcp_pitch` and `thumb_pip` joint ranges start at 0 rad (open). Setting `init_state.joint_pos` to 0 puts the joint AT the hard stop, causing PD oscillation against the joint limit at reset. Use a small offset (~0.05 rad / 3°) so the joint has wiggle room. Also: `curled_q = init_joint_pos`, so a 3° init means the curl regularizer rewards staying at 3° instead of impossible 0° (limit-bound).
- **Finger velocity_limit_sim sim2real gap**: previous fr3_tekken_left.py had `velocity_limit_sim=8.0` on mcp_pitch/yaw/pip groups (~460 deg/s). Real AgileHand hardware is ~30-60 deg/s. 8 rad/s let contact forces fling thumb_mcp_pitch backward by up to 7.6° in a single 60Hz step, causing buckling. **Use 1.0 rad/s** (~57 deg/s) — safer for hardware AND prevents single-step buckling. thumb_rot is already at 0.2618 rad/s (15 deg/s) and matches hardware spec.
...
- `revolute_thumb_mcp_pitch` and `revolute_thumb_pip` init at 0.0524 rad (3°) — moved off joint min (0 rad) 2026-05-11 to avoid PD controller oscillation against the hard stop. Also `curled_q = init_joint_pos`, so this makes the regularizer's "target" reachable instead of limit-bound. Other fingers' mcp_pitch stays at 0.1 rad as before.
- ~~`thumb_rot_init` EventTerm randomizes thumb rotation between -20° and 0° at reset~~ — **removed 2026-05-11 (run3f)**. The random offset prevented the policy from developing a consistent approach geometry. thumb_rot now always starts at -20° (joint min) deterministically.
