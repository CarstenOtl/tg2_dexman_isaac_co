...
- **Finger velocity_limit_sim — AgileHand hardware spec is 360 deg/s = 6.283 rad/s** for all non-thumb-rot finger joints (mcp_pitch, mcp_yaw, pip). Match the asset to this for sim2real accuracy. Current asset value of 8.0 rad/s (~458 deg/s) is only ~27% over spec, not catastrophic, but worth tightening to 6.283 rad/s when refining. Earlier note in this file (since corrected 2026-05-20) recommended "2.0 rad/s based on 30-60 deg/s hardware" — that was based on incorrect hardware-spec data; disregard. thumb_rot velocity stays at 0.2618 rad/s (15 deg/s) matching its separate hardware spec.
