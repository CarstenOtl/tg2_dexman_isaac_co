"""Script to perform SafeDagger student-teacher distillation for Kuka Allegro"""
...
parser = argparse.ArgumentParser(description="SafeDagger distillation for Kuka Allegro.")
...
def main(env_cfg, agent_cfg: dict):
    """ Performs SafeDagger distillation for Kuka Allegro. """
    ...
    if not args_cli.play_policy:
        if args_cli.teacher is not None:
            ...
            teacher_ckpt = os.path.join(parent_path, "pretrained_ckpts/kuka_allegro_teacher.pth")

    train_dir = "runs"
    experiment_name = (
        f"dextrah-kuka-allegro-safedagger-{vision_tag}-{arch_tag}"
